...
from ezdl.utils.grid import linearize, delinearize



class Optunizer:
    def __init__(self, study_name, grid, n_trials, storage_base="optuna" ,direction='maximize'):
        direction = "maximize" if direction == "max" else "minimize" if direction == "min" else direction
        self.study_name = study_name
        self.direction = direction
        self.n_trials = n_trials
        storage_name = f"sqlite:///{storage_base}/{study_name}.db"
        self.study = optuna.create_study(direction=direction, study_name=study_name, storage=storage_name, load_if_exists=True)
    ...
    def _init_grid(self, grid):
        ...
        self.hyperparameters = [
            (key, value) for key, value in self.lin_grid
            if len(value) > 1
        ]
        self.parameters = [
            (key, value[0]) for key, value in self.lin_grid
            if len(value) == 1
        ]
